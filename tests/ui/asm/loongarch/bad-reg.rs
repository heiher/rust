...
//@ min-llvm-version: 20
//@[loongarch32_ilp32d] compile-flags: --target loongarch32-unknown-linux-gnu
...
fn f() {
    ...
    unsafe {
        ...
        asm!("", out("$f0") _); // ok
        asm!("/* {} */", in(freg) f);
        //[loongarch32_ilp32s]~^ ERROR register class `freg` requires at least one of the following target features: d, f
